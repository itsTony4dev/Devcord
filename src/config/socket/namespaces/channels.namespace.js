...
export function initializeChannelsNamespace(io) {
  ...
  channelsNamespace.on("connection", async (socket) => {
    ...
    socket.on("disconnect", () => {
      ...
      Object.keys(channelUsers).forEach((channelId) => {
        if (
          ...
          channelUsers[channelId].includes(socket.user._id)
        ) {
          channelUsers[channelId] = channelUsers[channelId].filter(
            (id) => id !== socket.user._id
          );
    ...
    try {
      ...
      socket.emit("error", {
        message: "Failed to initialize workspace channels",
      });
    }
    ...
    socket.on("joinChannel", async ({ channelId }) => {
      try {
        ...
        const channelUsersList = await User.find({
          _id: { $in: channelUserIds },
    ...
    socket.on("joinWorkspace", async ({ workspaceId }) => {
      ...
      try {
        ...
        console.error(
          `Error joining workspace channels for ${workspaceId}:`,
          error
        );
      ...
      async ({
        ...
        try {
          ...
          const isChannelOwner =
            channel.createdBy.toString() === socket.user._id.toString();
          const isChannelMember = channel.allowedUsers.some(
            (id) => id.toString() === socket.user._id.toString()
          );
    ...
    socket.on("addReaction", async ({ messageId, channelId, reaction }) => {
      try {
        ...
        for (let i = message.reactions.length - 1; i >= 0; i--) {
          ...
          const userIndexInReaction = r.users.findIndex(
            (userId) =>
        ...
        if (!wasRemovingSameEmoji) {
          ...
          if (existingReactionIndex !== -1) {
            ...
            message.reactions[existingReactionIndex].users.push(
              socket.user._id
            );
          } else {
            // Create new reaction
          ...
          if (isChannelOwner) {
            ...
            for (const userId of channel.allowedUsers) {
              ...
              if (recipientSocketId) {
                channelsNamespace
                  .to(recipientSocketId)
                  .emit("messageDeleted", deletionData);
              }
            }
          } else {
